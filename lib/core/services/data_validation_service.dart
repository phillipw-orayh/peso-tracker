...
class DataValidationService {
  static bool validateExpense(Expense expense) {
    try {
      ...
      if (expense.dateTime.isAfter(DateTime.now().add(const Duration(days: 1)))) {
        debugPrint('❌ Invalid expense: Date cannot be more than 1 day in the future');
        return false;
      }
      
      // Check date is not too far in the past (more than 10 years)
      if (expense.dateTime.isBefore(DateTime.now().subtract(const Duration(days: 3650)))) {
  ...
  static bool validateSavingsGoal(SavingsGoal goal) {
    try {
      ...
      if (goal.targetAmount > 100000000) { // 100 million
      ...
      if (goal.currentAmount > goal.targetAmount && !goal.isCompleted) {
        debugPrint('⚠️ Warning: Current amount exceeds target but goal not marked completed');
      }
      ...
      if (goal.createdDate.isAfter(DateTime.now().add(const Duration(days: 1)))) {
      ...
      if (goal.isCompleted && goal.completedDate == null) {
        debugPrint('⚠️ Warning: Goal marked completed but no completion date set');
      }
  ...
  static bool validateUserData(UserData userData) {
    try {
      ...
      if (!['en', 'tl'].contains(userData.preferredLanguage)) {
        debugPrint('⚠️ Warning: Unsupported language: ${userData.preferredLanguage}');
      }
  ...
  static Future<ValidationResult> validateAllData() async {
    ...
    try {
      // Import database service dynamically to avoid circular imports
      final DatabaseService = (await import('database_service.dart')).DatabaseService;
      ...
      debugPrint('  - Expenses: ${result.validExpenses}/${result.totalExpenses} valid');
      debugPrint('  - Goals: ${result.validGoals}/${result.totalGoals} valid');
      debugPrint('  - User Data: ${result.isUserDataValid ? "Valid" : "Invalid"}');
...
class ValidationResult {
  ...
  double get goalValidityRate =>
      totalGoals > 0 ? validGoals / totalGoals : 1.0;
}