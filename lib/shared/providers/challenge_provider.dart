...
class ChallengeProvider extends ChangeNotifier {
  ...
  List<Challenge> get weeklyChallenges => 
      _availableChallenges.where((c) => c.type == ChallengeType.weekly).toList();
  
  List<Challenge> get monthlyChallenges => 
      _availableChallenges.where((c) => c.type == ChallengeType.monthly).toList();
  ...
  int get totalPoints => _earnedBadges.length * 50 + 
  ...
  Future<void> _updateUserStats() async {
    try {
      ...
      // Today's stats
      final todayExpenses = expenses.where((e) => _isSameDay(e.dateTime, now)).toList();
      ...
      // Weekly stats
      final weekExpenses = expenses.where((e) => e.dateTime.isAfter(startOfWeek)).toList();
      final weeklyGoalSavings = goals.fold(0.0, (sum, goal) {
        ...
        return sum + (goal.currentAmount * 0.1); // Assume 10% was added this week
      });

      // Monthly stats
      final monthExpenses = expenses.where((e) => e.dateTime.isAfter(startOfMonth)).toList();
      final monthlySpending = monthExpenses.fold(0.0, (sum, e) => sum + e.amount);
      final monthlySavings = goals.fold(0.0, (sum, goal) => sum + goal.currentAmount);
      final completedGoalsThisMonth = goals.where((g) => 
          g.isCompleted && 
      ...
      for (int i = 0; i < 365; i++) {
        final dayExpenses = expenses.where((e) => _isSameDay(e.dateTime, checkDate)).toList();
    ...
    for (final userChallenge in _enrolledChallenges) {
      ...
      if (isCompleted) {
        ...
        if (challenge.badgeReward != null) {
          ...
          final badge = ChallengeService.getAvailableBadges()[challenge.badgeReward!];
  ...
  Future<void> _updateDailyStreak(UserChallenge userChallenge) async {
    ...
    if (userData != null) {
      ...
      } else if (!_isSameDay(lastActive ?? now, now)) {
